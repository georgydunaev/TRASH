...
Fixpoint my_term_args (t:term) : list term.
Proof.
destruct t as [pt H].
destruct pt.
exact nil.
simpl in H.
revert H; move/andP => [H1 H2].
clear my_term_args f H1.

(*revert l H2. fix hy 1. intros ts H.
destruct ts.
exact []%list.
simpl in H.
revert H. move/andP => [H1 H2]. 
...
Defined.
exact (@minilemma []%list H2).
exact (@minilemma l H2).

 :=
  match  t with
  | Vari _     => [::]
  | Node fv ts => minilemma ts
  end.

End other.
unshelve simple eapply Term. exact p exact H0.

(* length (Node fv ts) = length ts *)
